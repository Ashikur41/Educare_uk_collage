...
class FrontendController extends Controller
{
    ...
    public function index(){
        ...
        if($setting->theme_version == 'theme9'){
            ...
            $data['courses'] = Course::where('status',1)->where('language_id', $currlang->id)->orderBy('serial_number', 'asc')->limit(6)->get();
    ...
    public function course() {
        ...
        $data['courses'] = Course::where('status',1)->where('language_id', $currlang->id)->orderBy('serial_number', 'asc')->get();
    ...
    public function course_details($slug){
        ...
        $data['course'] = Course::where('slug', $slug)->where('language_id', $currlang->id)->firstOrFail();
        $data['all_courses'] = Course::where('status', 1)->where('language_id', $currlang->id)->orderBy('serial_number', 'asc')->get();
   ...
   // Front Quote Page Function
    ...
    public function quote_submit(Request $request){
        ...
        $quote = new Quote();
        $quote->name = $request->course_applied_for;
        $quote->email = $request->email;
        $quote->phone = $request->middle_name;
        $quote->skypenumber = $request->gender_title;
        $quote->country = $request->first_name;
        $quote->budget = $request->last_name;
        $quote->file = $request->date_of_birth;
        $quote->subject = $request->nationality;
        $quote->description = $request->Sex;
        $quote->parsonel_address = $request->parsonel_address;
        $quote->city = $request->city;
        $quote->contact_country = $request->country;
        $quote->postcode = $request->postcode;
        $quote->contact_number = $request->contact_number;
        $quote->passport_number = $request->passport_number;
        $quote->passport_issue_date = $request->passport_issue_date;
        $quote->passport_expiry_date = $request->passport_expiry_date;
        $quote->place_of_issue = $request->place_of_issue;
        $quote->course_title = $request->course_title;
        $quote->institute_name = $request->institute_name;
        $quote->passing_year = $request->passing_year;
        $quote->grade = $request->grade;
        $quote->emergency_contact_name = $request->emergency_contact_name;
        $quote->relationship_name = $request->relationship_name;
        $quote->emergency_address = $request->emergency_address;
        $quote->emergency_postcode = $request->emergency_postcode;
        $quote->emergency_number = $request->emergency_number;
        // dd($quote);
        $quote->save();
